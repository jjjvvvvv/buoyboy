...
buoy_name_list = [name + "(" + str(buoy) + ")" for buoy, name in buoy_name_mapping.items()]
...
def new_buoy_data(selected_buoys, metric):
    ...
    for buoy in selected_buoys:
        ...
        data = ascii.read(
            f"https://www.ndbc.noaa.gov/data/5day2/{buoy}_5day.spec")
        ...
        while i < 24:
            ...
            # get buoy's swell period
                ...
                try:
                    df.loc[i, buoy] = pd.to_numeric(
                        data[i][7], errors='coerce')
                except ValueError:
                    pass
